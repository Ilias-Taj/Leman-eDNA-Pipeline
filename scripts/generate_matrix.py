...
Usage:
  python scripts/generate_matrix.py --paf out/alignments.paf --output out/species_counts.csv
...
def parse_paf(paf_path):
    # For each read, keep the best alignment by matching bases (col 10)
    best = {}
    with open(paf_path, "rt") as fh:
        for line in fh:
            if line.startswith("#") or not line.strip():
                continue
            cols = line.rstrip("\n").split("\t")
            # PAF fields (0-based indices):
            # 0: query_name, 1: query_len, 2: qstart, 3: qend, 4: strand,
            # 5: target_name, 6: target_len, 7: tstart, 8: tend, 9: n_match, 10: aln_len, 11: mapq
            if len(cols) < 11:
